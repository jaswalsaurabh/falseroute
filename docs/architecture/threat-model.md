...
The release does **not** claim to contain a real attacker or protect a production network. Live mutation is not implemented or enabled, and ADR-0005 activation evidence has not been recorded. Any future live mode would remain confined to dedicated demonstration resources, synthetic sources, bounded leases, and verified rollback.
...
FalseRoute currently uses one shared operator credential for the controlled demonstration. A valid bearer credential creates a short-lived signed browser session, and cookie-authenticated state changes require a session-bound CSRF token. This is demonstration access control, not production identity, account management, or tenant isolation.

### Control states

| State                      | Meaning                                                                                                                             |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **Implemented**            | Enforced by the current application or repository checks, with focused automated coverage.                                          |
| **Simulated**              | The workflow records an honest simulated result; it does not change cloud or customer resources.                                    |
| **Infrastructure-defined** | Expressed in checked templates or infrastructure configuration, but not evidence that a deployed environment currently enforces it. |
| **Activation-gated**       | Required before live use and not claimed as active until ADR-0005 activation evidence is recorded.                                  |

Current authentication, session, CSRF, validation, deterministic policy, redaction, durable audit, and process-local abuse controls are **Implemented**. Cloud Run decoy deployment, false-route assignment, and Cloud Armor mutation remain **Simulated**. Container and Cloud Run template constraints are **Infrastructure-defined**. Live cloud mutation, runtime IAM and edge controls, distributed quotas, production identity, and tenant isolation are **Activation-gated**. A requirement in the threat table does not by itself mean that an activation-gated control is active.
...
2. **Operator browser to web/API:** Browser input and API responses cross a client/server boundary. The controlled demonstration accepts the configured operator bearer credential or its short-lived signed session. This single-operator mechanism does not provide production identity or tenant isolation.
...
| Replay or duplicate events                        | Preserve event and correlation identifiers; use durable ingestion receipts and tool-operation keys so redelivery cannot duplicate a decision or recorded side effect                                                                                                                                                                                                                                     |
...
| Unauthenticated public access                     | Require the configured operator bearer credential or a valid short-lived signed session for protected routes. Keep the demo controlled because this shared-credential design is not production identity or tenant isolation.                                                                                                                                                                             |
...
| Cross-site request forgery                        | For the implemented signed-session flow, use an HttpOnly `SameSite=Lax` session cookie and require a matching, session-bound CSRF cookie and `X-CSRF-Token` header on state-changing requests. Bearer-authenticated requests do not rely on ambient browser credentials.                                                                                                                                 |
...
| SSE token exposure or connection exhaustion       | Authenticate SSE with either the `Authorization` bearer credential or the signed operator session cookie; never accept credentials in query parameters. Bound concurrent SSE clients, enforce heartbeat interval and disconnect cleanup, and redact payloads.                                                                                                                                            |
...
- The controlled demonstration has shared-credential operator authentication and signed browser sessions. Production identity, account lifecycle, tenant isolation, and real networking are not present.
...
Any live action requires separate adapter implementation, the accepted ADR-0005 boundary, a completed activation record, named owners, isolation and failure evidence, and successful activation gates. Expanding beyond the dedicated demonstration project, owned gateway, allowlisted resources, or synthetic sources requires another threat-model revision and ADR.
